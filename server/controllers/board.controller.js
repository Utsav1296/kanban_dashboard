...
const getFavouritesBoard = async (req, res) => {
  try {
    const favourites = await Board.find({
      user: req.user._id,
      favourite: true
...
const updateFavouriteBoardPosition = async (req, res) => {
  const { boards } = req.body
  try {
    for (const key in boards.reverse()) {
      const board = boards[key]
...
export { createBoard, getAllBoard, updateBoardPosition, getOneBoard, updateBoard, getFavouritesBoard, updateFavouriteBoardPosition, deleteBoard }